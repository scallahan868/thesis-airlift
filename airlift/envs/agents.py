...
class EnvAgent:
    ...
    def step(self, action, cargo_by_id: Dict[CargoID, Cargo], elapsed_time) -> Tuple[dict, List[str]]:
        ...
        # Print whether the action was legal (no warnings) or not (warnings present)
        if warnings:
            print(f"[EnvAgent] Agent action ILLEGAL or PARTIALLY ILLEGAL: {warnings}")
        else:
            print(f"[EnvAgent] Agent action LEGAL: {updated_action}")
