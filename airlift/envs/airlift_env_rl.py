...
class AirliftEnv(ParallelEnv):
    ...
    REWARD_CARGO_MISSED_PENALTY = 20
    REWARD_CARGO_LATE_PENALTY = 0.05
    REWARD_MOVEMENT_PENALTY = 0.001

    REWARD_CARGO_PICKUP = 0
    REWARD_PROXIMITY_BONUS = 0.5
    REWARD_CARGO_DELIVERY = 50.0
    REWARD_ON_TIME_BONUS = 15.0
    EPISODE_REWARD_DELIVERY_SCALE = 0
    REWARD_SHARED_DELIVERY_FRACTION = 0.05
    ...
    def calculate_rewards(self, actions, prev_cargo_delivered, dones, infos):
        """
        Calculate rewards for all agents with improved incentives for active behavior,
        while minimizing per-step overhead.

        Perf tricks:
        - Use local aliases for hot paths (fewer getattr dict lookups).
        - Cache flight_time / flight_cost with small dicts on self.
        - Merge per-agent passes where possible (movement + pickup + proximity).
        - Keep/set cached 'delivered' set correctly each step.
        - Keep object identity stable (no copying of spaces here).
        """
        # --------- hot local aliases & constants (avoid repeated lookups) ----------
        agents = self.agents
        _agents = self._agents
        REWARD_MOVEMENT_PENALTY   = self.REWARD_MOVEMENT_PENALTY
        REWARD_CARGO_PICKUP       = self.REWARD_CARGO_PICKUP
        REWARD_PROXIMITY_BONUS    = self.REWARD_PROXIMITY_BONUS
        REWARD_CARGO_DELIVERY     = self.REWARD_CARGO_DELIVERY
        REWARD_ON_TIME_BONUS      = self.REWARD_ON_TIME_BONUS
        REWARD_CARGO_MISSED_PENALTY = self.REWARD_CARGO_MISSED_PENALTY
        REWARD_CARGO_LATE_PENALTY   = self.REWARD_CARGO_LATE_PENALTY
        EPISODE_REWARD_DELIVERY_SCALE = self.EPISODE_REWARD_DELIVERY_SCALE

        routemap = self.routemap
        get_flight_time = routemap.get_flight_time
        get_flight_cost = routemap.get_flight_cost

        PlaneState_MOVING = PlaneState.MOVING
        PlaneState_PROC   = PlaneState.PROCESSING
        NOAIRPORT = NOAIRPORT_ID

        current_step = self._elapsed_steps
        max_cycles = self.max_cycles
        cargo_list = self.cargo
        cargo_by_id = self.cargo_by_id

        # Rewards must start at zero for every agent
        rewards = {agent: 0.0 for agent in agents}

        # ---------------------- tiny memo caches for this env ----------------------
        # (persist across steps; clear if topology changes)
        if not hasattr(self, "_ft_cache"):
            self._ft_cache = {}   # (src, dst, plane_type) -> time
        if not hasattr(self, "_fc_cache"):
            self._fc_cache = {}   # (src, dst, plane_type) -> cost

        ft_cache = self._ft_cache
        fc_cache = self._fc_cache

        def flight_time(src, dst, ptype):
            key = (src, dst, ptype)
            v = ft_cache.get(key)
            if v is None:
                try:
                    v = get_flight_time(src, dst, ptype)
                except (KeyError, ValueError):
                    v = float("inf")
                ft_cache[key] = v
            return v

        def flight_cost(src, dst, ptype):
            key = (src, dst, ptype)
            v = fc_cache.get(key)
            if v is None:
                try:
                    v = get_flight_cost(src, dst, ptype)
                except (KeyError, ValueError):
                    v = 0.0
                fc_cache[key] = v
            return v

        # --------------------- pass 1: agent-local rewards ------------------------
        # Combine: movement penalty + pickup reward + proximity bonus
        for agent in agents:
            a = _agents[agent]
            state = a.state
            ptype = a.plane_type
        ...
        # Efficient "delivered set" maintenance:
         # --------------------- pass 2: deliveries & global penalties --------------
        # Compute the set delivered *now* (fresh every step)
        current_delivered = {c.id for c in cargo_list if self.cargo_delivered(c)}

        # Use prev_cargo_delivered passed in from step() (delivered before this step)
        prev_delivered_input = prev_cargo_delivered or set()
        newly_delivered = current_delivered - prev_delivered_input

        if newly_delivered:
            for cargo_id in newly_delivered:
                c = cargo_by_id.get(cargo_id)
                if not c:
                    continue

                # Heuristic: who delivered? check airport match for any plane
                delivering_agent = None
                end_ap = c.end_airport
                for agent in agents:
                    a = _agents[agent]
                    if a.current_airport == end_ap or a.previous_airport == end_ap:
                        delivering_agent = agent
                        break

                if delivering_agent is not None:
                    reward = REWARD_CARGO_DELIVERY
                    if c.delivery_time <= c.soft_deadline:
                        reward += REWARD_ON_TIME_BONUS
                    rewards[delivering_agent] += reward

                    shared_frac = getattr(self, "REWARD_SHARED_DELIVERY_FRACTION", 0.0)
                    if shared_frac and shared_frac > 0.0:
                        shared_amount = reward * float(shared_frac)
                        for ag in agents:
                            if ag != delivering_agent:
                                rewards[ag] += shared_amount
                else:
                    # fallback: small shared credit if nobody clearly owns it
                    shared = REWARD_CARGO_DELIVERY * 0.2
                    for agent in agents:
                        rewards[agent] += shared

        # Maintain the cache for *next* step
        self._last_delivered_check = current_delivered

        # (5) Cargo penalties (missed / late). Iterate once over cargo list.
        for c in cargo_list:
            if self.cargo_missed(c) and current_step == c.hard_deadline + 1:
                # Missed exactly at the first step after hard_deadline
                pen = REWARD_CARGO_MISSED_PENALTY
                for agent in agents:
                    rewards[agent] -= pen
            elif current_step > c.soft_deadline and not self.cargo_delivered(c):
                # One-time late penalty
                if not hasattr(c, "_late_penalty_applied"):
                    pen = REWARD_CARGO_LATE_PENALTY
                    for agent in agents:
                        rewards[agent] -= pen
                    c._late_penalty_applied = True

        # (6) Early completion bonus (once everything is done)
        # if all(dones.values()):
        #     if all(self.cargo_done(c) for c in cargo_list):
        #         # proportional to unused horizon
        #         early_bonus = 20.0 * (max_cycles - current_step) / max_cycles
        #         if early_bonus > 0:
        #             for agent in agents:
        #                 rewards[agent] += early_bonus

        # Maintain your delivered cache for next step (so line above gets cheaper)
        # If your env updates delivered state elsewhere, replace this with that set.
        self._last_delivered_check = {c.id for c in cargo_list if self.cargo_delivered(c)}

        if all(dones.values()):
            total_cargo = len(cargo_list)
            delivered = sum(1 for c in cargo_list if self.cargo_delivered(c))
            frac_delivered = delivered / max(total_cargo, 1)
    ...
    def reset(self, seed=None) -> Dict:
        ...
        # self.possible_agents: List[AgentID] = ['a_' + str(i) for i in range(self.world_generator.num_agents)]
    ...
    def step(self, actions):
        ...
        agents_snapshot = tuple(self.agents)

        prev_cargo_delivered = {c.id for c in self.cargo if self.cargo_delivered(c)}
        ...
        self._update_state_and_obs(new_cargo)
         ...
         # Build new obs BEFORE we mutate self.agents so shape matches return
        obs = self.observe()

        # Compute final rewards ONCE for this step (single source of truth)
        rewards = self.calculate_rewards(actions, prev_cargo_delivered, self.dones, self.info)

        # Ensure we return rewards for the agents that were active at the start of this step
        self.rewards = {a: rewards.get(a, 0.0) for a in agents_snapshot}

        # Update total rewards AFTER final per-step rewards are set
        for a in agents_snapshot:
            self.total_rewards[a] += self.rewards[a]
        ...
        # self.rewards = self.calculate_rewards(actions, prev_cargo_delivered, self.dones, self.info)
    ...
    def metrics(self):
        ...
        metrics = Metrics(
            ...
            proportion_deliveries_missed = missed_deliveries / float(total_cargo_generated)
        )
        return metrics
