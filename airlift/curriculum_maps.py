...
class DifficultyProgressionMap(AirliftCurriculumMap):
    """
    Starts with easier scenarios and progressively increases difficulty.
    Based on the assumption that lower testids are easier.
    """

    def __init__(self, seed=None):
        super().__init__(seed=seed)
        self.name = "DifficultyProgressionMap"
        self.current_difficulty_level = 0  # 0-14 corresponding to testids
        self.current_testid = 0
        self.current_seed = seed or 12345
    ...
    def refresh(self):
        # Read the single source of truth each episode/reset
        # self.current_seed = (self.seed or 12345) + self.reset_counter
